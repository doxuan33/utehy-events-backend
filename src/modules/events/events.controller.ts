...
export const eventsController = {
  ...
  async getEvents(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      // For PAGE_ADMIN, pass the user's page_id for security
      const userPageId = req.user?.role === 'PAGE_ADMIN' ? req.user?.page_id : undefined;
      // [BUG FIX] Truyền userId để student feed lọc đúng theo trang đã follow
      const result = await eventsService.getEvents(parsed.data, req.user?.role, userPageId, req.user?.id);
  ...
  async createEvent(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      // For PAGE_ADMIN, ignore client-sent page_id and use user's page_id for security
      let pageIdToUse;
      if (req.user?.role === 'PAGE_ADMIN' && req.user?.page_id) {
        pageIdToUse = req.user.page_id;
      } else {
      ...
      const result = await eventsService.createEvent(pageIdToUse, { ...parsed.data, banner_url: bannerUrl });
